...
//Termino de la creación del objeto conexión
  ...
  //Se agregan las fechas en un datetime para saber la diferencia de días entre cada fecha seleccionada.
  $dateFecha1 = new DateTime($fechaInicio);
  $dateFecha2 = new DateTime($fechaFin);
  $diferenciaFechas = $dateFecha1->diff($dateFecha2);
  $diasDiferenciaFechas = $diferenciaFechas->format('%a');
  echo ' 
  <div id="diasSugerido" name="diasSugerido">
         <div class="col-md-3"></div>
           <div class="col-md-2">
                <font size="4" color="black">Días para sugerido</font>
            </div>
          <div class="col-md-3">
             <input type="text" readonly="readonly" value= "'. $diasDiferenciaFechas.' días" disabled="disabled" class="form-control" name="dias"  id="dias" />
          </div>
    </div>
<br><br><br><br><br>
...
//CONEX2 alamacena la conexión a otro servidor, ya que conex almacena la conexión local.
...
$consulta = $conex->consultarByConex($conex2,$sqlNombre);
  ...
  //Se hace el procedimiento para consultar los SKU en todas las sucursales.
    if($nombreDB == "all") {
    try{
      if($conex->consultar($sqlSucursales) != null) {
        ...
        //Validación para la conexión a otro servidor  
          ...
          if($consulta != null) {
            foreach ($consulta as $row) {
              ...
              $sugerido = round($row['Total de Venta'],2) / $diasDiferenciaFechas * $_POST['diasProveedor'] - round($row['Existencias']);
              ...
              echo '<td><p align="center">'.round($sugerido,2).'</p></td>';
              echo '</tr>';
              ...
              $objeto->setSugerido(round($sugerido,2));
