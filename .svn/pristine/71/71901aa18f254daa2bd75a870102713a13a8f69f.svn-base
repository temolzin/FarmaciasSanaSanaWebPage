...
//Variables
  ...
  $_SESSION['fechaInicioCortes'] = $fechaInicio;
  $_SESSION['fechaFinCortes'] = $fechaFin;

  $nombreDB = filter_var($_POST['sucursales'], FILTER_SANITIZE_STRING);
  $_SESSION['nombreDBCortes'] = $nombreDB;
  ...
  echo '
    <div class="panel panel-default">
      ...
      <table class="table table-striped table-bordered table-hover" id="dataTables-example">
        <thead>
          <tr>
            <th align="center">Fecha del corte</th>
            <th align="center">Total del corte</th>
          </tr>
        </thead>
        <tbody>';
    ...
    try{
        ...
        if($conex->consultarByConex($conex2,$sql) != null){
          foreach ($conex->consultarByConex($conex2,$sql) as $row) {
            ...
            echo '<td><p align="center"> $ '.round($row['cierre_Total'],2).'</p></td>';
            echo '</tr>';
          }
         } else{
		      echo '<tr class="gradeA">';
